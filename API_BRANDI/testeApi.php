...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'PUT':
        ...
        $sql = "UPDATE cliente SET nome='$nome', categoria='$categoria', preco='$preco', pc='$pc', lc='$lc', qe='$qe', c='$c' WHERE idcli=$id";
    ...
    case 'POST':
        ...
        $sql = "INSERT INTO cliente (nome, categoria, preco, pc, lc, qe, c) VALUES ('$nome','$categoria','$preco','$pc','$lc','$qe','$c')";
        if ($conn->query($sql) === TRUE) {
            if ($conn->affected_rows > 0) {
    ...
    case 'GET':
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $response[] = ['id' => $row['idcli'], 'nome' => $row['nome'], 'categoria' => $row['categoria'],'preco' => $row['preco'], 'pc' => $row['pc'], 'lc' => $row['lc'], 'qe' => $row['qe'], 'c' => $row['c'],];
            }
        } else {
